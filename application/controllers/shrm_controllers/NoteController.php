...
class NoteController extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('shrm_models/Note', 'Note');
    ...
    public function index()
    {
        try {
            ...
            $this->load->view('shrm_views/pages/note/index', $data);
        } catch (Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            if (in_array($this->session->userdata('role'), $allowed_roles)) {
                $this->load->view('shrm_views/pages/note/create');
            } else {
                redirect('note');
            }
        } catch (Exception $e) {
    ...
    public function store()
    {
        try {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $this->load->view('shrm_views/pages/note/create', $data);
                return;
            }
            ...
            $noteData = [
                'reporting_officer_id' => $this->session->userdata('ro_id'),
                'user_id' => $user_id,
    ...
    public function edit($id)
    {
        try {
            ...
            $this->load->view('shrm_views/pages/note/edit', $data);
        } catch (Exception $e) {
            $this->session->set_flashdata('error', 'Something went wrong. Please try again.');
            redirect('note');
        }
            ...
            if (!empty($_FILES['attachments']['name'][0])) {
                ...
                for ($i = 0; $i < $totalFiles; $i++) {
                    ...
                    if ($this->upload->do_upload('single_file')) {
                        ...
                        // Get attachment title or use original filename
                        $attachmentTitle = !empty($attachmentTitles[$i]) ? $attachmentTitles[$i] : $uploadData['orig_name'];
                        ...
                        $this->Note->add_attachment($id, $attachmentData);
                    } else {
                        $uploadErrors = $this->upload->display_errors('', '');
                        $this->session->set_flashdata('error', 'File upload failed for "' . $fileData['name'] . '": ' . $uploadErrors);
                        $this->shrm->trans_rollback();
                        redirect('note/edit/' . $id);
                        return;
                    }
                }
            }
    ...
    public function delete()
    {
        try {
            ...
            $this->shrm->where('id', $id)->update('notes', [
            ...
            return $this->output->set_content_type('application/json')->set_output(json_encode(['success' => false, 'message' => 'Something went wrong.']));
        }
    }
    ...
    public function view($id)
    {
        try {
            ...
            $this->load->view('shrm_views/pages/note/view', $data);
        } catch (Exception $e) {
            $this->session->set_flashdata('error', 'Something went wrong. Please try again.');
            redirect('note');
        }
    ...
    public function add_discussion()
    {
        try {
            ...
            $inserted = $this->shrm->insert('note_discussions', $data);
