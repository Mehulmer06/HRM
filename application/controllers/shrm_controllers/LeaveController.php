...
class LeaveController extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('shrm_views/pages/leave/index', $data);
    }
    ...
    public function applyLeave()
    {
        ...
        $roID = $this->session->userdata('ro_id');
    ...
    public function updateLeave()
    {
        ...
        $existing = $this->shrm->get_where('leave_requests', ['id' => $leaveId])->row();
        ...
        if ($totalDays > 0) {
            ...
            // Update request
            $this->shrm->where('id', $leaveId)->update('leave_requests', $leaveData);

            // Delete old days and insert new
            $this->shrm->delete('leave_request_days', ['leave_request_id' => $leaveId]);
            foreach ($days as $day) {
                $day['leave_request_id'] = $leaveId;
                $this->shrm->insert('leave_request_days', $day);
            }
    ...
    public function take_action()
    {
        ...
        $leave = $this->shrm->get_where('leave_requests', ['id' => $leaveId])->row();
        if (!$leave) {
        ...
        $this->shrm->where('id', $leaveId)->update('leave_requests', $updateData);
    ...
    {
        $userId = $this->session->userdata('user_id');
        $leave = $this->shrm->get_where('leave_requests', ['id' => $id])->row();
        ...
        $days = $this->shrm->get_where('leave_request_days', ['leave_request_id' => $id])->result();

        foreach ($days as $d) {
            if ($d->leave_type == 'CL' && $d->cl_grant_id) {
                $this->shrm->set('is_used', 'n')->where('id', $d->cl_grant_id)->update('cl_grants');
            } elseif ($d->leave_type == 'Extra' && $d->extra_day_id) {
                $this->shrm->set('is_used', 'n')->where('id', $d->extra_day_id)->update('extra_day_requests');
            }
        }

        $this->shrm->delete('leave_request_days', ['leave_request_id' => $id]);
        $this->shrm->delete('leave_requests', ['id' => $id]);
    ...
    public function cancel()
    {
        ...
        if ($cancelType === 'full') {
            ...
            $alreadyRequested = $this->shrm->get_where('leave_cancellation_requests', [
            ...
            $this->shrm->insert('leave_cancellation_requests', [
                'leave_request_id' => $leaveId,
            ...
            foreach ($selectedDays as $dayId) {
                ...
                $exists = $this->shrm->get_where('leave_cancellation_requests', [
                    'leave_request_id' => $leaveId,
    ...
    public function cancel_action()
    {
        ...
        // Get the leave_request_id from the cancel_id
        ...
        // Get all pending cancellation requests for this leave_request_id
        $allCancelIds = $this->Leave->getCancellationIdsByLeaveRequest($cancelRequest->leave_request_id);

        if (empty($allCancelIds)) {
            $this->session->set_flashdata('error', 'No pending cancellation requests found.');
            redirect('leave');
        }

        // Get all cancellation requests for processing
        $this->shrm->select('*');
        $this->shrm->from('leave_cancellation_requests');
        $this->shrm->where('leave_request_id', $cancelRequest->leave_request_id);
        $this->shrm->where('status', 'pending');
        $allCancelRequests = $this->shrm->get()->result();

        // If approved, process all cancellations for this leave request
        if ($action === 'approved') {
            foreach ($allCancelRequests as $request) {
                $this->processCancellationApproval($request);
            }
        }

        // Update all cancellation request statuses for this leave request
        $this->shrm->where('leave_request_id', $cancelRequest->leave_request_id);
        $this->shrm->where('status', 'pending');
        $this->shrm->update('leave_cancellation_requests', [
            'status' => $action,
            'ro_id' => $userId,
        ...
        $cancelCount = count($allCancelRequests);
        $this->session->set_flashdata('success', "Cancellation request with {$cancelCount} day(s) has been {$action} successfully.");
        redirect('leave');
    }
    ...
    private function cancelFullLeave($leaveId)
    {
        ...
        $this->shrm->where('id', $leaveId)->update('leave_requests', [
            'status' => 'cancelled',
            'total_days' => 0,
            'cl_used' => 0,
            'extra_used' => 0,
