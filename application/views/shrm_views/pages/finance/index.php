	...
	<div class="d-flex justify-content-between align-items-start">
		<div>
			...
			<nav class="breadcrumb-nav">
				<a href="<?= base_url('shrm/dashboard') ?>">Dashboard</a> / <span class="text-muted">Finance</span>
			</nav>
		</div>
							...
							<div class="form-group">
								...
								<select class="form-select <?= form_error('month') ? 'is-invalid' : '' ?>" name="month"
									...
									<?php foreach ($months as $key => $month): ?>
										<option
											value="<?= $key . '' . $yearShort ?>"><?= $month . "'" . $yearShort ?></option>
									<?php endforeach; ?>
								</select>
					...
					<?php if (isset($other_documents) && !empty($other_documents)): ?>
						...
						<?php foreach ($other_documents as $document): ?>
							<tr>
								...
								<td>
									<a href="<?= base_url('upload/finance_other_document/' . urlencode($document['document_file'])); ?>"
									   download
									   class="action-btn btn-edit"
									   title="Download">
										<i class="fas fa-download"></i>
